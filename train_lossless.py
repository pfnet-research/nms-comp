...
import dataset as D
from model_lossless import LosslessModel


def run_training(args):

    model = LosslessModel(c_list=args.c_list, q_num=args.q_num, out=args.out)
    if args.gpu >= 0:
    ...
    trainer.extend(extensions.LogReport(trigger=log_interval, log_name='log_lossless'))
    trainer.extend(extensions.snapshot_object(model, 'model_all'), trigger=(args.iteration, 'iteration'))

    trainer.extend(extensions.PrintReport([
        'iteration',
        'main/mean_entropy',
        'val/main/mean_entropy',
        'elapsed_time',
    ]))
